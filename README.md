...
- **Note Recognition:** Tone Map teaches exact sound-to-key and sound-to-voice recall over the full 88-key piano through a cumulative progressive map. It starts with six evenly spaced landmarks in the middle octave, fills the six notes between them, then alternates into neighboring octaves only after the active map is stable. Each randomized evidence round spans the entire widening pool before starting another round, so six added tones never replace or starve familiar ones. The final challenge keeps all 88 keys in the mix indefinitely, without immediate pitch repeats when another note is available. Every piano key remains labeled for context while the prompt target and target marker stay hidden until commitment. Keyboard and vocal evidence remain independent; every active tone must prove a fresh three-answer blind streak before the map can widen. Mistakes retain lifetime history while revoking only the affected streak, and explicit advancement is never timed. Its isolated prompt is the canonical indefinite **Play / Stop** lane; its separate Simon transport locks answers until the authored sequence finishes, then leaves recall untimed. The laboratory also includes same/different, direction, reference-backed navigation, pitch class, octave-only, complete note, octave family, and cross-timbre practice.

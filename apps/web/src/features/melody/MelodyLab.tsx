...
import { playFrequencies, playPitchContour } from "@/audio/synth";
...
export function MelodyLab() {
  ...
  const playPhrase = (notes = phrase) => void playFrequencies(notes.map(continuousMidiToHz), "sequential", { timbre, duration: .48, amplitude: .25 });
  ...
  return (
    <div className="page melody-page">
      ...
      <Panel className="melody-config"><Segmented value={mode} onChange={setMode} options={[{ value: "echo", label: "Call & response" }, { value: "contour", label: "Contour" }, { value: "draw", label: "Pitch drawing" }, { value: "transcribe", label: "Transcription" }]} /><div className="melody-fields"><Select label="Notes" value={length} onChange={(event) => { const value = Number(event.target.value); setLength(value); setPhrase(generatePhrase(value, chromatic, rootMidi)); }}><option value="2">2 notes</option><option value="3">3 notes</option><option value="4">4 notes</option><option value="6">6 notes</option><option value="8">8 notes</option></Select><Switch label="Allow chromatic notes" checked={chromatic} onChange={(value) => { setChromatic(value); setPhrase(generatePhrase(length, value, rootMidi)); }} /></div></Panel>
      ...
      {mode === "draw" && <div className="draw-workspace">
        <Panel className="draw-stage"><div className="panel-heading"><div><Eyebrow>Voice is continuous</Eyebrow><h2>Draw a vocal gesture.</h2></div><button className="text-button" onClick={() => setDrawnPoints([])}>Clear</button></div><div className="pitch-canvas"><div className="draw-labels"><span>high</span><span>center</span><span>low</span></div><svg ref={drawingRef} viewBox="0 0 600 220" preserveAspectRatio="none" onPointerDown={(event) => { setDrawing(true); setDrawnPoints([]); event.currentTarget.setPointerCapture(event.pointerId); }} onPointerMove={addDrawPoint} onPointerUp={() => setDrawing(false)}><defs><pattern id="draw-grid" width="50" height="36.6" patternUnits="userSpaceOnUse"><path d="M50 0H0V36.6" /></pattern><linearGradient id="draw-line"><stop stopColor="#63d7ff" /><stop offset=".5" stopColor="#d8ff3e" /><stop offset="1" stopColor="#ff6b45" /></linearGradient></defs><rect width="600" height="220" fill="url(#draw-grid)" /><line x1="0" x2="600" y1="110" y2="110" />{drawPath ? <><path d={drawPath} className="drawn-glow" /><path d={drawPath} className="drawn-line" /></> : <text x="300" y="118" textAnchor="middle">press · draw · hear · embody</text>}</svg></div><div className="draw-actions"><ActionButton disabled={drawnMidi.length < 2} onClick={() => playPitchContour(drawnMidi, 2.8)}><Icon name="play" size={17} /> Synthesize drawing</ActionButton><ActionButton className="primary" disabled={drawnMidi.length < 2} onClick={() => { setSelectedMidi(Math.round(drawnMidi[0] ?? 60)); setView("mirror"); }}><Icon name="mic" size={17} /> Reproduce it</ActionButton></div></Panel>
        <Panel className="manifest-note"><span className="manifest-symbol">∿</span><Eyebrow>Visual → auditory → motor</Eyebrow><h2>A contour is a plan for motion.</h2><p>The synthesizer keeps this drawing continuous. It is not rounded to piano keys before you hear it.</p><dl><div><dt>Start</dt><dd>{drawnMidi[0] == null ? "—" : noteLabel(Math.round(drawnMidi[0]))}</dd></div><div><dt>Range</dt><dd>{drawnMidi.length ? `${(Math.max(...drawnMidi) - Math.min(...drawnMidi)).toFixed(1)} st` : "—"}</dd></div><div><dt>Samples</dt><dd>{drawnMidi.length}</dd></div></dl></Panel>
      </div>}

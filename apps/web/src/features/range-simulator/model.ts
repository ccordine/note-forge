...
export const RANGE_SIMULATOR_MIN_MIDI = 36;
export const RANGE_SIMULATOR_MAX_MIDI = 83;
...
function latestRatingsByMidi(session: Readonly<RangeSimulatorSessionState>): Map<number, EffortRating> {
  ...
  for (const observation of session.observations) {
    if (observation.task.kind === "baseline-candidate") continue;
    ratings.set(observation.task.midi, observation.rating);
  }
  return ratings;
}
...
/**
 * Share only like-for-like user claims with Range Loop. Comfort ratings stay in
 * the simulator; they are not clean-phonation or pitch-accuracy evidence.
 */

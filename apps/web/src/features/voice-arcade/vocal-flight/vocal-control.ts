...
function resolveOptions(options: Readonly<VocalControlOptions>): ResolvedVocalControlOptions {
  ...
  if (resolved.responsePerSecond <= 0 || resolved.responseCurve <= 0) {
    throw new RangeError("Control response and curve must be positive.");
  }
...
function asymmetricAxis(delta: number, negativeExtent: number, positiveExtent: number): number {
  return clampSignedUnit(delta / (delta < 0 ? negativeExtent : positiveExtent));
}
...
function shapeAxis(
  ...
  const normalized = clampUnit((magnitude - exitThreshold) / (1 - exitThreshold));
  return {
...
function immediatelyInactive(
  ...
  const vector = Object.freeze({
    ...emptyVector(),
    pitchConfidence: clampUnit(Number.isFinite(sample.confidence) ? sample.confidence : 0),
    brightnessConfidence: clampUnit(
...
export function updateVocalControl(
  ...
  if (!continuity.accepted) {
    return immediatelyInactive(state, sample, 0, state.lastAuthority);
  }
  ...
  if (!voiced) return immediatelyInactive(state, sample, deltaSeconds, continuity.authority);
  const pitchConfidence = clampUnit(Number.isFinite(sample.confidence) ? sample.confidence : 0);
  const brightnessConfidence = clampUnit(
    Number.isFinite(sample.brightnessConfidence) ? sample.brightnessConfidence : 0,
  );
  ...
  const brightnessAxis = brightnessReliable
    ...
    : 0;
  const vector = Object.freeze({
    pitchAxis: clampSignedUnit(Math.abs(pitchAxis) < EPSILON ? 0 : pitchAxis),
    brightnessAxis: clampSignedUnit(Math.abs(brightnessAxis) < EPSILON ? 0 : brightnessAxis),
    pitchConfidence,
    brightnessConfidence,
    voiced: true,
    active: true,

...
| Detector/engine PCM tests | Direct per-window YIN candidates, target-independent temporal tracking, anti-aliased high-rate normalization, configured rates/range, adversarial spectra and noise strengths, octave-identity regressions, silence, and seeded-noise behavior for deterministic sample arrays | Browser capture, permission, worklet delivery, React ownership, or rendered UI |
